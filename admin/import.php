<?php
    ...
    function parse_tag($str, $type, $i){
        ...
        foreach($strtags as $strtag){
			...
			elseif(get_class($nodes) == 'SimpleXMLElement'){
				$add = true;
				foreach($attributes as $n=>$attr){
					$attr = $names[$n];
					if(!check_condition($nodes->attributes()->$attr, $operators[$n], $values[$n])){
						$add = false;
						break;
					}
				}
				if($add)$data .= get_text($nodes)."\n";
			}
            $data = preg_replace("/\n$/", '', $data);
    ...
    function parse_field($field, $i){
		if(substr( $field['value'], 0, strlen("<![CDATA[") ) === "<![CDATA[" && substr( $field['value'], strlen($field['value'])-strlen("]]>"), strlen($field['value']) ) === "]]>")
		{
			//$field['value'] = str_replace('<![CDATA[', '', $field['value']);
